...
# klubspecifikke
b_i <- fixef(fe_perform2) - a
b_bourne <- 0.9167350
beta_i <- fixef(covid_fe_revenue2) - alpha
beta_bourne <- 1.1265645
...
# Beregning
P_hat <- 45 / (1+exp(-(a + b_i + c * (alpha+beta_i) + c * log(R) - c * log(W)) /(c*gamma -1) ))
P_hat



45 / (1+exp(-(a + b_bourne + c * (alpha+beta_bourne) + c * log(R) - c * log(W)) /(c*gamma -1) ))
...
### Funktions-kode
```{r}
profit <- 45 / (1+exp(-(a + b_i + c * (alpha+beta_i) + c * log(Rt_bar) - c * log(Wt_bar) + c * log(c*gamma) ) / (c*gamma -1) ))
profit
