...
a <- mean(b_i)        
alpha <- mean(beta_i)     
c <- 1.496608      # estimat fra perform regression på log_wage
gamma <- 0.172558       # estimat fra revenue regression på log_rank
...
# Beregning
P_hat <- X / (1 + exp(
  (-a - b_i - c * (alpha + beta_i) - c * log(Rt_bar) + c * log(Wt_bar)) / (c * gamma - 1)
))
...
### Funktions-kode
```{r}
45 / (1+exp(-(a + b_i + c * (alpha+beta_i) + c * log(Rt_bar) - c * log(Wt_bar) + c * log(c*gamma) ) / (c*gamma -1) ))

P_star <- X / (1 + exp(
  (-a - b_i - c * (alpha + beta_i) - c * log(Rt_bar) + c * log(Wt_bar) + c * log(c * gamma)) / (c * gamma - 1)
))
