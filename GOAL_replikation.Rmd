...
covid_pdata <- pdata.frame(covid_df, index =c("club", "season_numeric")) #hele datasæt - covid
...
alpha <- 0  
c <- summary(perform_dummy2)$coefficients["log_wage", "Estimate"]      # estimat fra perform regression på log_wage
gamma <- summary(revenue_uden)$coefficients["log_rank", "Estimate"]       # estimat fra revenue regression på log_rank
Rt_bar <- mean(covid_pdata$revenue)   # Gennemsnitlig omsætning for alle klubber over hele perioden
Wt_bar <- mean(covid_pdata$wage)   # Gennemsnitlig lønudgift

# klubspecifikke
b_i <- fixef(perform_dummy2)
beta_i <- fixef(revenue_uden)
...
positioner_uden_tilskuere$win <- 45 / (1+exp(-(a + b_i + c * (alpha+beta_i) + c * log(Rt_bar) - c * log(Wt_bar)) /(c*gamma -1) ))
positioner_uden_tilskuere$profit <- 45 / (1+exp(-(a + b_i + c * (alpha+beta_i) + c * log(Rt_bar) - c * log(Wt_bar) + c * log(c*gamma) ) / (c*gamma -1) ))
