...
### Performance
```{r}
fe_perform <- plm(log_rank ~ log_wage + lag(log_rank, 1) + log_goals, data = data_filtered,
  model = "within"  # eller "pooling", "random", etc. afhængigt af hvad du laver
)

re_perform <- plm(log_rank ~ log_wage + lag(log_rank, 1) + log_goals, data = data_filtered,
...
ols_perform <- plm(log_rank ~ log_wage + lag(log_rank, 1) + log_goals, data = data_filtered, model = "pooling")
...
# FE eller OLS?
ols_revenue <- plm(log_revenue ~ log_rank + lag(log_revenue, 1) + log_tilskuere, data = data_filtered, model = "pooling")
